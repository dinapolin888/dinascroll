...
export const FollowSetupModal: React.FC<FollowSetupModalProps> = ({
  ...
  const sym = trade?.symbol && trade.symbol !== 'Unknown' ? trade.symbol : 'XAUUSD';
  const calculatedTP = isBuy 
    ? +(entry + (riskAmount * rrMultiplier)).toFixed(sym === 'EURUSD' ? 4 : 2)
    : +(entry - (riskAmount * rrMultiplier)).toFixed(sym === 'EURUSD' ? 4 : 2);

  // Estimated Risk in USD = Risk in price * Lot * contract multiplier (Forex: 100,000, Gold: 100, Crypto: 1)
  const contractMult = sym.includes('XAU') || sym.includes('GOLD') ? 100 : (sym.includes('BTC') || sym.includes('ETH') || sym.includes('CRYPTO')) ? 1 : 100000;
  const estimatedRiskUSD = +(riskAmount * lot * contractMult).toFixed(2);
